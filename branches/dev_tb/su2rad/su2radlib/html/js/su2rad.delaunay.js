...
//------------------------------------------------------------
// DelaunayVertex class
//------------------------------------------------------------

function DelaunayVertex( x, y, z )
{
	this.x = x;
	this.y = y;
	this.z = z;
...
function DelaunayTriangle( v0, v1, v2 )
{
	this.v0 = v0;
	this.v1 = v1;
	this.v2 = v2;
        this.z  = (v0.z + v1.z + v2.z) / 3.0
...
function DelaunayTriangulate( vertices )
{
	...
	for( i in vertices )
	{
		...
		AddDelaunayVertex( vertex, triangles );
	}
...
function CreateBoundingTriangle( vertices )
{
	...
	var stv0 = new DelaunayVertex( minx - dx,   miny - dy*3 );
	var stv1 = new DelaunayVertex( minx - dx,   maxy + dy   );
	var stv2 = new DelaunayVertex( maxx + dx*3, maxy + dy   );
...
function AddDelaunayVertex( vertex, triangles )
{
	...
	edges = DelaunayUniqueEdges( edges );
...
function DelaunayUniqueEdges( edges )
{
	// TODO: This is O(n^2), make it O(n) with a hash or some such
	var uniqueDelaunayEdges = [];
	for( var i in edges )
	{
		var edge1 = edges[i];
		var unique = true;
		...
		if( unique )
		{
			uniqueDelaunayEdges.push( edge1 );
		}
	}

	return uniqueDelaunayEdges;
